%{
    ...
    extern map<string,vector<string>>funcParam;
    extern map<string,string> returnType;

    int strCount = 0;                       // Counter for string constants h
%}
...
%token ARROW 
%token LTE GTE DOUBLE_EQUALS NOT_EQUAL 
...
multiplicative_expression: 
        ...
        | multiplicative_expression '%' unary_expression
        {
            ...
            // Assign the result of the '%' to the higher data type
            DataType final = ((one->type.type > two->type.type) ? (one->type.type) : (two->type.type));
            $$->loc = ST->gentemp(final);                       // Store the final result in a temporary
...
additive_expression: 
        ...
        | additive_expression '-' multiplicative_expression
        {
            ...
            // Assign the result of the '-'ion to the higher data type
            DataType final = ((one->type.type > two->type.type) ? (one->type.type) : (two->type.type));
            $$->loc = ST->gentemp(final);                       // Store the final result in a temporary
...
conditional_expression: 
        ...
        {   
            ...
            */
            ...
            backpatch(temp, nextinstr);         // Backpatch with nextinstr
        }
        ;
...
direct_declarator: 
        ...
        | direct_declarator '(' parameter_type_list_opt ')'
        {
            ...
            if((int)paramList.size() == 0) funcParam[$1->name].push_back("void");
            else if($1->name == "printInt") 
            {
                funcParam[$1->name].push_back("int");
                returnType[$1->name] = "int";
            }
            else if($1->name == "printStr") 
            {
                funcParam[$1->name].push_back("char*");
                returnType[$1->name] = "int";
            }
            else if($1->name == "readInt") 
            {
                funcParam[$1->name].push_back("int*");
                returnType[$1->name] = "int";
            }
            else
            {
                for(int i = 0; i < (int)paramList.size(); i++)
                {
                    ...
                    if(curParam->type.type == ARRAY) {          // If the parameter is an array
                        if(curParam->type.nextType == CHAR) funcParam[$1->name].push_back("char[]");
                        else if(curParam->type.nextType == INT) funcParam[$1->name].push_back("int[]");
                    }
                    else if(curParam->type.type == POINTER) {   // If the parameter is a pointer
                        cout<<"I am pointer\n";
                        if(curParam->type.nextType == INT) funcParam[$1->name].push_back("int*");
                        else if(curParam->type.nextType == CHAR) funcParam[$1->name].push_back("char*");
                    }
                    else {
                        if(curParam->type.type == VOID)
                            funcParam[$1->name].push_back("void");
                        else if(curParam->type.type == CHAR)
                            funcParam[$1->name].push_back("char");
                        else if(curParam->type.type == INT)
                            funcParam[$1->name].push_back("int");
                    }
                }
            }
...
jump_statement: 
        ...
        | RETURN_ expression ';'
        {
            if(ST->lookup("RETVAL")->type.type == ST->lookup($2->loc)->type.type) {
                emit($2->loc, "", "", RETURN);      // Emit the quad when return type is '!' void
            }
            $$ = new expression();
        }
        ;
...
function_prototype:
        type_specifier declarator
        {
            ...
            if($1 == 0) returnType[$2->name] = "void";
            else if($1 == 2) returnType[$2->name] = "char";
            else if($1 == 3) returnType[$2->name] = "int";
